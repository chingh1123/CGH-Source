...
const { i } = require('../controller/function')
const WIDTH = 15;
const HEIGHT = 10;
const gameBoard = [];
const apple = { x: 1, y: 1 };
...
class SnakeGame {
    ...
    async startGame() {
        ...
        let up = new MessageButton().setEmoji(emojis.up ? emojis.up : '🢁').setStyle('PRIMARY').setCustomId(btn_up)
        let left = new MessageButton().setEmoji(emojis.left ? emojis.left : '🢀').setStyle('PRIMARY').setCustomId(btn_left)
        let down = new MessageButton().setEmoji(emojis.down ? emojis.down : '🢃').setStyle('PRIMARY').setCustomId(btn_down)
        let right = new MessageButton().setEmoji(emojis.right ? emojis.right : '🢂').setStyle('PRIMARY').setCustomId(btn_right)
    ...
    async gameOver(msg) {
        ...
        return await msg.edit({ embeds: [editEmbed], components: [] }).catch()   
    }
